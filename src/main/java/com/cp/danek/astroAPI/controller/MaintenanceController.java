...
public class MaintenanceController {
    ...
    public ResponseEntity<ApiResponse<List<MaintenanceDTO>>> getAllMaintenances() {
        try {
            ...
            return ResponseEntity.ok(ApiResponse.success("Maintenances retrieved successfully", maintenances));
        } catch (Exception e) {
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
                    .body(ApiResponse.error("Error retrieving maintenances: " + e.getMessage()));
        }
    }
    ...
    public ResponseEntity<ApiResponse<MaintenanceDTO>> getMaintenanceById(@PathVariable Long id) {
        try {
            ...
            return ResponseEntity.ok(ApiResponse.success("Maintenance retrieved successfully", modelMapper.toMaintenanceDTO(maintenance)));
        } catch (RuntimeException e) {
            return ResponseEntity.status(HttpStatus.NOT_FOUND)
                    .body(ApiResponse.error(e.getMessage()));
        } catch (Exception e) {
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
                    .body(ApiResponse.error("Error retrieving maintenance: " + e.getMessage()));
        }
    }

    // POST /api/maintenances - создать новое обслуживание
    @PostMapping
    public ResponseEntity<ApiResponse<MaintenanceDTO>> createMaintenance(@RequestBody CreateMaintenanceDTO request) {
        try {
            ...
            return ResponseEntity.status(HttpStatus.CREATED)
                    .body(ApiResponse.success("Maintenance created successfully", modelMapper.toMaintenanceDTO(createdMaintenance)));
        } catch (RuntimeException e) {
            return ResponseEntity.status(HttpStatus.BAD_REQUEST)
                    .body(ApiResponse.error(e.getMessage()));
        } catch (Exception e) {
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
                    .body(ApiResponse.error("Error creating maintenance: " + e.getMessage()));
        }
    }

    // PUT /api/maintenances/{id} - обновить обслуживание
    @PutMapping("/{id}")
    public ResponseEntity<ApiResponse<MaintenanceDTO>> updateMaintenance(@PathVariable Long id, @RequestBody CreateMaintenanceDTO request) {
        try {
            ...
            return ResponseEntity.ok(ApiResponse.success("Maintenance updated successfully", modelMapper.toMaintenanceDTO(updatedMaintenance)));
        } catch (RuntimeException e) {
            return ResponseEntity.status(HttpStatus.NOT_FOUND)
                    .body(ApiResponse.error(e.getMessage()));
        } catch (Exception e) {
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
                    .body(ApiResponse.error("Error updating maintenance: " + e.getMessage()));
        }
    }
    ...
    public ResponseEntity<ApiResponse<Void>> deleteMaintenance(@PathVariable Long id) {
        try {
            ...
            return ResponseEntity.ok(ApiResponse.success("Maintenance deleted successfully", null));
        } catch (Exception e) {
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
                    .body(ApiResponse.error("Error deleting maintenance: " + e.getMessage()));
        }
    }

    // GET /api/maintenances/telescope/{telescopeId} - обслуживания телескопа
    @GetMapping("/telescope/{telescopeId}")
    public ResponseEntity<ApiResponse<List<MaintenanceDTO>>> getMaintenancesByTelescope(@PathVariable Long telescopeId) {
        try {
    ...
    public ResponseEntity<ApiResponse<List<MaintenanceDTO>>> getMaintenancesByEngineer(@PathVariable Long engineerId) {
        try {
    ...
    public ResponseEntity<ApiResponse<List<MaintenanceDTO>>> getMaintenancesByStatus(@PathVariable MaintenanceStatus status) {
        try {
    ...
    public ResponseEntity<ApiResponse<List<MaintenanceDTO>>> getMaintenancesByType(@PathVariable MaintenanceType type) {
        try {
    ...
    public ResponseEntity<ApiResponse<MaintenanceDTO>> startMaintenance(@PathVariable Long id) {
        try {
            maintenanceService.startMaintenance(id);
            Maintenance maintenance = maintenanceService.getMaintenanceById(id)
                    .orElseThrow(() -> new RuntimeException("Maintenance not found with id: " + id));
            return ResponseEntity.ok(ApiResponse.success("Maintenance started successfully", modelMapper.toMaintenanceDTO(maintenance)));
        } catch (RuntimeException e) {
            return ResponseEntity.status(HttpStatus.NOT_FOUND)
                    .body(ApiResponse.error(e.getMessage()));
        } catch (Exception e) {
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
                    .body(ApiResponse.error("Error starting maintenance: " + e.getMessage()));
        }
    }

    // PATCH /api/maintenances/{id}/complete - завершить обслуживание
    @PatchMapping("/{id}/complete")
    public ResponseEntity<ApiResponse<MaintenanceDTO>> completeMaintenance(
        ...
        try {
            ...
            Maintenance maintenance = maintenanceService.getMaintenanceById(id)
                    .orElseThrow(() -> new RuntimeException("Maintenance not found with id: " + id));
            return ResponseEntity.ok(ApiResponse.success("Maintenance completed successfully", modelMapper.toMaintenanceDTO(maintenance)));
        } catch (RuntimeException e) {
            return ResponseEntity.status(HttpStatus.NOT_FOUND)
                    .body(ApiResponse.error(e.getMessage()));
        } catch (Exception e) {
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
                    .body(ApiResponse.error("Error completing maintenance: " + e.getMessage()));
        }
    }

    // PATCH /api/maintenances/{id}/cancel - отменить обслуживание
    @PatchMapping("/{id}/cancel")
    public ResponseEntity<ApiResponse<MaintenanceDTO>> cancelMaintenance(@PathVariable Long id) {
        try {
            maintenanceService.cancelMaintenance(id);
            Maintenance maintenance = maintenanceService.getMaintenanceById(id)
                    .orElseThrow(() -> new RuntimeException("Maintenance not found with id: " + id));
            return ResponseEntity.ok(ApiResponse.success("Maintenance cancelled successfully", modelMapper.toMaintenanceDTO(maintenance)));
        } catch (RuntimeException e) {
            return ResponseEntity.status(HttpStatus.NOT_FOUND)
                    .body(ApiResponse.error(e.getMessage()));
        } catch (Exception e) {
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
                    .body(ApiResponse.error("Error cancelling maintenance: " + e.getMessage()));
        }
    }
    ...
    public ResponseEntity<ApiResponse<List<MaintenanceDTO>>> getMaintenanceHistory(@PathVariable Long telescopeId) {
        try {
            List<MaintenanceDTO> maintenances = maintenanceService.getMaintenanceHistory(telescopeId).stream()
                    .map(modelMapper::toMaintenanceDTO)
                    .collect(Collectors.toList());
            return ResponseEntity.ok(ApiResponse.success("Maintenance history retrieved successfully", maintenances));
        } catch (Exception e) {
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
                    .body(ApiResponse.error("Error retrieving maintenance history: " + e.getMessage()));
        }
    }

    // GET /api/maintenances/upcoming - предстоящие обслуживания
    @GetMapping("/upcoming")
    public ResponseEntity<ApiResponse<List<MaintenanceDTO>>> getUpcomingMaintenances() {
        try {
            List<MaintenanceDTO> maintenances = maintenanceService.getUpcomingMaintenances().stream()
                    .map(modelMapper::toMaintenanceDTO)
                    .collect(Collectors.toList());
            return ResponseEntity.ok(ApiResponse.success("Upcoming maintenances retrieved successfully", maintenances));
        } catch (Exception e) {
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
                    .body(ApiResponse.error("Error retrieving upcoming maintenances: " + e.getMessage()));
        }
    }

    // GET /api/maintenances/active - активные обслуживания
    @GetMapping("/active")
    public ResponseEntity<ApiResponse<List<MaintenanceDTO>>> getActiveMaintenances() {
        try {
            List<MaintenanceDTO> maintenances = maintenanceService.getActiveMaintenances().stream()
                    .map(modelMapper::toMaintenanceDTO)
                    .collect(Collectors.toList());
            return ResponseEntity.ok(ApiResponse.success("Active maintenances retrieved successfully", maintenances));
        } catch (Exception e) {
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
                    .body(ApiResponse.error("Error retrieving active maintenances: " + e.getMessage()));
        }
    }
    ...
    public ResponseEntity<ApiResponse<Double>> getTotalMaintenanceCost(@PathVariable Long telescopeId) {
        try {
            Double totalCost = maintenanceService.getTotalMaintenanceCost(telescopeId);
            return ResponseEntity.ok(ApiResponse.success("Total maintenance cost retrieved successfully", totalCost));
        } catch (Exception e) {
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
                    .body(ApiResponse.error("Error retrieving total maintenance cost: " + e.getMessage()));
        }
    }
}